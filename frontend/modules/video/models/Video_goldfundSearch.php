...
class Video_goldfundSearch extends Video {
    ...
    public function rules() {
        return [
            ...
            [['title', 'embed', 'description', 'alias', 'ids', 'conspects', 'id_training', 'password', 'tags', 'preview', 'author'], 'safe'],
        ];
    }
    ...
    public function search($params, $i = 0, $p = false) {
        ...
        if ($p === true) {
            ...
            $is_buy = $query->joinWith(['videoUsr'])->andWhere([VideoUsr::tableName() . '.user_id' => Yii::$app->user->id])->count();
            $free = $query->where(['val' => NULL, 'type_id' => $i, 'gp' => 1])->count();
